...
def tkmGetCustomerFormData(startDate, endDate, variant, online):
    ...
    # "PERSONALIZED"
    # "NEW"
    url = settings.TKM_WEBSERVICE_ADDRESS + \
        'tkmGetCustomerRequest_to_personalize_' + settings.TKM_WEBSERVICE_VERSION
    params = {
        "queryParams": [{
            "name": "online",
            "value": str(online)}, {
    ...
    req = requests.post(url=url, data=json.dumps(params), headers=headers, auth=(
        settings.TKM_WEBSERVICE_USER_LOGIN, settings.TKM_WEBSERVICE_USER_PASS))
...
def tkmPdfMultipleExport(request):
    ...
    if isAutorized(request.user, permisions):
        ...
        title = 'Eksport ' + startDate + '-' + endDate + ', wariant ' + \
            variant + ', czy online ' + str(online) + '.pdf'
        bufferedPdf = None
        try:
...
def generateOnePagePdf(p, data, id):
    ...
    p.drawString(TEXT_X, TEXT_Y,
                 data['name'].upper()+'  '+data['surname'].upper())
...
def pdfExport(name, surname, id, title, text):
    ...
    if settings.FONTS:
        pdfmetrics.registerFont(
            TTFont('Arial-bold', settings.FONTS+'arial-bold.ttf'))
...
def pdfMultiPagesExport(title, data):
    ...
    for d in data['response']:
        ...
        if id:
            ...
            id = str(d['birthdate'])[0:FOREIGNER_ID_LENGTH] + \
                '_' + d['name'][0:1].upper()
            fileName = getPhotoName(id)
        # Dodanie daty utworzenia pliku w celu sortowania
        if fileName != None and os.path.exists(fileName):
            d['fileDate'] = os.path.getctime(fileName)
            d['fileId'] = id
            persons.append(d)
        else:
            lostPhotos.append(id + '.jpg - ' + d['name'] + ' ' + d['surname'])
    # sortowanie po dacie pliku
    if settings.SORT_MULTIPLE_PRINT_WITH_FILE_DATE :
        persons.sort(key=lambda s: s['fileDate'])
    # wydruk
    for person in persons:
        generateOnePagePdf(p, person, person['fileId'])
